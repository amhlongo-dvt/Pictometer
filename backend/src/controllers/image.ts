...
export function createImageApp(
    ...
    imageApp.post(IMAGE_ROUTE_GENERATE, async (c) => {
        ...
        const { imageId: sourceImageId, message } = await c.req.json<{ imageId?: string, message: string }>();

        let b64Image: string;
        if (sourceImageId) {
            // Get the source image from S3 for editing
            const sourceImageMetadata = await imageResource.get(sourceImageId);
            if (!sourceImageMetadata) {
                return c.json({ error: "Source image not found" }, 404);
            }

            const sourceImageFile = client.file(sourceImageMetadata.s3key);
            const sourceImageBuffer = Buffer.from(await sourceImageFile.arrayBuffer());

            b64Image = await generateEditMessageResponse(message, sourceImageBuffer);
        } else {
            b64Image = await generateMessageResponse(message);
        }
        ...
        const imageId = randomUUIDv7();
        const s3key = `${userId}/${imageId}-generated-image.png`;
        ...
        await client.write(s3key, imageBuffer, { type: "image/png" })
